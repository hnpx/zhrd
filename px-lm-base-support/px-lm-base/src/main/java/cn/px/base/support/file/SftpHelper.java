...
//					ex.printStackTrace();
     ...
     * @return
     */
    public FileVo upload1(File file, String toPath, boolean sFlag, boolean ftpEnable, boolean synFlag) {
        FileVo fvo = new FileVo();

        //String fileType = FileTypeUtil.getType(file);
        String fileType =getTypePart(file.getName());
        String fileName = UUID.fastUUID().toString(true) + "." + fileType;
        try {
            // 上传原图
            fvo.setPath(upload(file, toPath, fileName, ftpEnable, synFlag));
