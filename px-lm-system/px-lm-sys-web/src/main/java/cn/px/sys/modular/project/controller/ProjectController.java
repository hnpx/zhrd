...
public class ProjectController extends BaseController<ProjectEntity, ProjectService> {
    ...
    public Object query(@RequestParam(required = false) String name,@RequestParam(required = false) String contactPerson,
                        @RequestParam(required = false) String phone,@RequestParam(required = false) Integer timeStatus,
      ...
      Page<Map<String,Object>> project = super.service.getListPage(name,cid,contactPerson,phone,timeStatus,month);
    ...
    public Object update3(ProjectEntity params){
    ...
    @PostMapping("/stop")
    @ResponseBody
    public Object stop(ModelMap modelMap, ProjectEntity params) {
        //TODO 数据验证
        Long userId = LoginContextHolder.getContext().getUserId();
        if(userId==1){
