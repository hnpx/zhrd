...
public class Uploader {
	...
	private String[] allowFiles = { ".rar", ".doc", ".docx", ".zip", ".pdf", ".txt", ".swf", ".wmv", ".gif", ".png",
			".jpg", ".jpeg", ".bmp", "xlsx", "xls", "mp4", "avi", "pdf" };
	// 文件大小限制，单位KB
	...
	public void upload(MultipartFile upfile, String funType) throws Exception {
		...
		try {
			...
			this.ppp = "files/upload/" + funType + "/" + this.fileName;
			BufferedInputStream in = new BufferedInputStream(upfile.getInputStream());
			this.ufile = new File(this.url);
			if (!this.ufile.getParentFile().exists()) {
				this.ufile.getParentFile().mkdirs();
			}
			FileOutputStream out = new FileOutputStream(this.ufile);
			this.setFilepath(this.ufile.toString());

			BufferedOutputStream output = new BufferedOutputStream(out);
			Streams.copy(in, output, true);
			this.state = this.errorInfo.get("SUCCESS");
			this.size = this.ufile.length();
