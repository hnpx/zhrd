...
public class LoginController extends BaseController {
    ...
    public Object loginVali(@RequestParam(value = "code", required = false) String code,
            ...
            try {
                ...
                try {
                    ...
                    if (allUser == null) {
                        ...
                        allUser.setNickname(nickname);
                        allUser.setAvatar(avatar);
                        allUser.setAddress("-" + "-");
                        //TODO 获取用户的头像和昵称信息
            ...
            redisHelper.set(token, allUser.getId(), 24 * 60 * 60);
            /**
             * 用户登录成功
             */
            if (token != null) {
        ...
        @ApiOperation("解密手机号")
        @ResponseBody
        public Object loginVali1 (@RequestParam(value = "code", required = false) String code,
            ...
            try {
                ...
                try {
                    ...
                    // WxMaUserInfo wxMaUserInfo = wxService.getUserService().getUserInfo(session.getSessionKey(),encryptedData,iv);
                    JSONObject jsonObject = JSONUtil.parseObj(phone);
                    System.out.println(jsonObject.get("phoneNumber"));
                    String phoneStr = "";
                    if (jsonObject.get("phoneNumber") != null) {
                        phoneStr = jsonObject.get("phoneNumber") + "";
                    } else {
                        phoneStr = "";
                    }
                    allUser = this.allUserService.readByOpenid(wxOpenid);
                    allUser.setPhone(phoneStr);
                    this.allUserService.update(allUser);
                } catch (WxErrorException e) {
                    this.logger.error(e.getMessage(), e);
                }
         ...
         * @return
         */
        @GetMapping(value = "/wx/login/phone")
        @ApiOperation("解密手机号")
        @ResponseBody
        public Object loginVali2 (@RequestParam(value = "code", required = false) String
        code, @RequestParam(value = "encryptedData", required = false) String encryptedData,
                @RequestParam(value = "iv", required = false) String iv){
            ...
            try {
                ...
                try {
                    ...
                    this.logger.error(e.getMessage(), e);
                    return new SuccessResponseData(201, "", "");
                }
            } catch (Exception e) {
                e.printStackTrace();
                return new SuccessResponseData(201, "", "");
